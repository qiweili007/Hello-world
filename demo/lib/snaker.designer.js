(function(snaker){
    ...
    designer.util = {
        ...
        tip : function(rect, name) {
        	...
        	if(!matched) {
        		$("#tipDIV").hide();
        		$("#"+designer.config.tipId).hide();
        		return;
        	}
        	if(designer.config.tipId == null || designer.config.tipId == undefined || designer.config.tipId==""){
                $.ajax({
                    type:'GET',
                    url:designer.config.ctxPath+"/Snaker/tasktipJson",
                    ...
                    success: function(data){
                    	//alert(data.operator + " " +data.task_name + " " + data.create_time);
                    	...
                    	$("#tipDIV").show();
                    	//document.getElementById("tipDIV").style.zIndex="11";
                    	$("#taskId").html(data.task_name);
                    	$("#operator").html(data.operator);
                    	$("#arriveTime").html(data.create_time);
                    }
                });
        	}
        	else{
	        	var options = {target:"#"+designer.config.tipId,
	        			url:designer.config.ctxPath+"/Snaker/tasktip",
	        			data:{orderid:designer.config.orderId,taskname:name}
	        	};
	        	//alert(designer.config.tipId);
        		$("#"+designer.config.tipId).show();
	        	$("#"+designer.config.tipId).bjuiajax('doLoad', options);
        	}
    ...
    designer.path=function(o, r, from, to){
        ...
        _text.drag(function(r,o){
                if(!designer.config.dragable){return}
	...
	designer.init=function(c, o){
		var _w = 930/*snaker(window).width()*/, _h = 715/*snaker(window).height()*/, _r = Raphael(c,_w,_h)/*_r = Raphael(c,_w*1.5,_h*1.5)*/, _states = {}, _paths = {};
